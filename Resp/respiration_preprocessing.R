#calculation of mean and standard deviation of the respiratory data within 30s intervals

rm(list=ls())
setwd("C:\\Users\\jihe5432\\Dropbox\\Stockholms Universitet\\Sleepy brain pilot\\Eileen kod\\sleepybrain2-eileen\\Resp")


#read in respiratory data
files_respiration <- list.files(recursive = T)
...
respiration$subject <- as.integer(substr(respiration$file, 5, 8))
respiration$session <- as.integer(substr(respiration$file, 14, 14))


#set condition for each participant "manually"
respiration$condition <- "fullsleep"
respiration$condition[respiration$subject == 8001 & respiration$session == 2] <- "sleepdeprived"
respiration$condition[respiration$subject == 8008 & respiration$session == 3] <- "sleepdeprived"
respiration$condition[respiration$subject == 8016 & respiration$session == 2] <- "sleepdeprived"
respiration$condition[respiration$subject == 8042 & respiration$session == 3] <- "sleepdeprived"
respiration$condition[respiration$subject == 8064 & respiration$session == 3] <- "sleepdeprived"
respiration$condition[respiration$subject == 8078 & respiration$session == 3] <- "sleepdeprived"
respiration$condition[respiration$subject == 8052 & respiration$session == 3] <- "sleepdeprived"
respiration$condition[respiration$subject == 8047 & respiration$session == 3] <- "sleepdeprived"
respiration$condition[respiration$subject == 8041 & respiration$session == 2] <- "sleepdeprived"

# Quality control based on visual inspection
# Reject the following recordings
respiratione$included <- TRUE
respiration$included[heartrate$subject == 8078 & heartrate$session == 3] <- FALSE # Odd readings, likely due to missing data
respiration$included[heartrate$subject == 8064 & heartrate$session == 2] <- FALSE # Missing data
respiration$included[heartrate$subject == 8064 & heartrate$session == 3] <- FALSE # Missing data
...
for(i in 1:length(files_respiration)){ 
  ...
  #plot(MeanBreath, type="l", main="Mean of rpm")
  
  
  #creates dataframe containing breathing data within 30s intervals for each participant
  if(!exists("rpm_mean_data")){
    rpm_mean_data <- data.frame(resprate_mean=MeanBreath,resprate_sd=SdBreath,subject=breathing_recording$subject[1], session=breathing_recording$session[1], condition=breathing_recording$condition[1], index=1:length(MeanBreath))
  } else {
    thesedata <- data.frame(resprate_mean=MeanBreath,resprate_sd=SdBreath,subject=breathing_recording$subject[1], session=breathing_recording$session[1], condition=breathing_recording$condition[1], index=1:length(MeanBreath))
    rpm_mean_data <- rbind(rpm_mean_data, thesedata)
  }
}

write.csv2(rpm_data, "C:\\Users\\jihe5432\\Dropbox\\Stockholms Universitet\\Sleepy brain pilot\\Eileen kod\\sleepybrain2-eileen\\rpm_data.csv", row.names = FALSE)
write.csv2(rpm_mean_data, "C:\\Users\\jihe5432\\Dropbox\\Stockholms Universitet\\Sleepy brain pilot\\Eileen kod\\sleepybrain2-eileen\\rpm_mean_data.csv", row.names = FALSE)
